                ...
                <tr>
                    ...
                    <td id="status-{{ $id }}">
                        @if($bookRequest->status == config('setting.active'))  
                            <img id="btn_change_status_{{ $id }}" onclick="return changeStatus({{ $bookRequest->id }}, 1);" class="approve-request"  src="{{ config('setting.path_img_active') }}" >
                        @else
                            <img id="btn_change_status" onclick="return changeStatus({{ $bookRequest->id }}, 0);" class="approve-request" src="{{ config('setting.path_img_deactive') }}" >
                        @endif
                    </td>
                    <td>
